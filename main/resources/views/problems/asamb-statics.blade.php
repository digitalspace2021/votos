...
@section('cabecera')
    <div class="pricing-header p-3 pb-md-4 mx-auto text-center">
        ...
        <div class="col-12">
            <label for="asamb_id" class="form-label">Asambleista :

                @foreach ($asambleistas as $item)
                    @if ($item->id == $asamb)
                        <label>
            ...
            <select class="form-control" name="asamb_id" id="asamb_id" onchange="getStatitics({{$asamb}})" required>
                <option value="" selected>Seleccione el asambleista</option>
                @foreach ($asambleistas as $item)
                    <option value="{{$item->id}}" {{$item->id == $asamb ? 'selected' : ''}} >
                        {{ $item->nombres }} {{ $item->apellidos }}
                    </option>
                @endforeach
            </select>
...
@section('cuerpo')
    ...
    <div id="container2" class="city" style=""></div>

    <div id="container3" class="city" style="display:none"></div>

    <div id="container4" class="city" style="display:none">
        <select class="form-select" name="vereda_id" id="vereda_id"  onchange="getStatitics({{$asamb}})">
    ...
    <div id="container5" class="city" style="display:none">
        <select class="form-select" name="barrio_id" id="barrio_id"  onchange="getStatitics({{$asamb}})">
        ...
        async function getStatitics(can) {
            ...
            if ($('#asamb_id').val() !="") {
              if($('#vereda_id').val() == "" && $('#barrio_id').val() == "" )  {
                window.location.href = '/estadisticas/asambleistas/' + $('#asamb_id').val();
              }
              if($('#vereda_id').val() != "" && $('#barrio_id').val() == "" )  {
                window.location.href = '/estadisticas/asambleistas/' + can + '/ver/' + $('#vereda_id').val();
              }
              if($('#vereda_id').val() == "" && $('#barrio_id').val() != "" )  {
                window.location.href = '/estadisticas/asambleistas/' + can + '/bar/' + $('#barrio_id').val();
              }
